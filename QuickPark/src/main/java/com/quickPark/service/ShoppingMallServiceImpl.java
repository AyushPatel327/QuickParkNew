...
public class ShoppingMallServiceImpl implements ShoppingMallService {
		...
		if (mallRepository.findAll().isEmpty()) {
			...
			for (Login login : loginRepository.findAll()) {
				if (login.getEmail().equals(user.getEmail()) && login.getPassword().equals(user.getPassword())
						&& login.getRole().toLowerCase().equals(user.getEmail().toLowerCase())) {
					status = "success";
					break;
				}
			}
	...
	@Override
	public int getShoppingMallByEmail(String mallEmail) {

		return mallRepository.getShoppingMallIdByMallEmail(mallEmail);
	}
